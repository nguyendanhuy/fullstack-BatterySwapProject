      ...
      <div className="container mx-auto p-6">
        ...
        <Card className="border-0 shadow-lg bg-white overflow-hidden hover:shadow-xl transition-all duration-300 mb-8">
          ...
          <CardContent>
            ...
            return (<Card key={station.id} className={`border-l-4 transition-all duration-300 hover:scale-105 ${isEmpty ? 'border-l-red-500 bg-red-50' :
                    ...
                    <CardContent className="p-4">
                      <div className="space-y-2">
                        <div className="flex items-center justify-between">
                          ...
                          <Badge variant={isEmpty ? "destructive" :
                    isFullyStaffed ? "default" :
                        ...
                        <div className="space-y-1">
                          <div className="flex justify-between text-sm">
                            ...
                            <span className={`font-medium ${isEmpty ? 'text-red-600' :
                    isFullyStaffed ? 'text-green-600' :
                        'text-yellow-600'}`}>
                          ...
                          <div className="w-full bg-gray-200 rounded-full h-2">
                            <div className={`h-2 rounded-full transition-all duration-300 ${isEmpty ? 'bg-red-500' :
                    isFullyStaffed ? 'bg-green-500' :
                        'bg-yellow-500'}`} style={{ width: `${(stationStaffCount / station.maxStaff) * 100}%` }}></div>
                          </div>
                          <div className="space-y-1">
                            {staffList
                    .filter(staff => staff.station === station.name)
                    .map(staff => (<div key={staff.id} className="flex items-center text-xs text-gray-600">
                                  <div className="w-2 h-2 bg-blue-500 rounded-full mr-2"></div>
                                  {staff.name}
                                </div>))}
                            {stationStaffCount === 0 && (<div className="text-xs text-gray-500 italic">Chưa có nhân viên</div>)}
                          </div>
                        </div>
                      </div>
                    </CardContent>
        ...
        <Card className="border-0 shadow-lg bg-white overflow-hidden hover:shadow-xl transition-all duration-300">
          ...
          <CardContent>
            <Table>
              ...
              <TableBody>
                {staffList.map((staff) => (<TableRow key={staff.id}>
                    <TableCell className="font-medium">{staff.name}</TableCell>
                    <TableCell>{staff.email}</TableCell>
                    <TableCell>{staff.phone}</TableCell>
                    <TableCell>
                      {staff.station ? (<div className="flex items-center">
                          <MapPin className="h-4 w-4 mr-1 text-electric-blue"/>
                          {staff.station}
                        </div>) : (<span className="text-muted-foreground">Chưa phân công</span>)}
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      <div className="flex space-x-2">
                        ...
                        {staff.station ? (<Button size="sm" variant="outline" onClick={() => handleUnassignStaff(staff.id)}>
                            Hủy assign
                          </Button>) : (<Button size="sm" onClick={() => openAssignDialog(staff)}>
                            Assign
                          </Button>)}
                      </div>
                    </TableCell>
        ...
        <Dialog open={isAssignDialogOpen} onOpenChange={setIsAssignDialogOpen}>
          <DialogContent>
            ...
            .map((station) => (<SelectItem key={station.id} value={station.id.toString()}>
